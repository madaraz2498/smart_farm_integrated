...
class DashboardStats {
  ...
  factory DashboardStats.fromJson(Map<String, dynamic> json) {
    ...
    return DashboardStats(
      ...
      aiServicesOnline:_i(j['ai_services_online'] ?? j['active_services'] ?? 6),
      mostUsedService: j['most_used_service'] as String? ?? j['top_service'] as String? ?? 'Plant Disease',
      analysesGrowth:  j['analyses_growth']   as String? ?? '+0%',
      usersGrowth:     j['users_growth']       as String? ?? '+0%',
    );
  }

  final int    totalAnalyses, totalUsers, activeUsers, aiServicesOnline;
...
class AdminNavItem {
  const AdminNavItem({required this.icon, required this.label, this.isAdminOnly = false});
