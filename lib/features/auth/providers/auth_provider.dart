...
class AuthProvider extends ChangeNotifier {
  ...
  Future<void> _init() async {
    try {
      ...
      if (u != null) { _user = u; _status = AuthStatus.authenticated; }
      else            { _status = AuthStatus.unauthenticated; }
    } catch (_) { _status = AuthStatus.unauthenticated; }
    notifyListeners();
  }
  ...
  Future<AuthResult> login({required String email, required String password}) async {
    ...
    _begin();
    try {
      final r = await _svc.login(email: email.trim().toLowerCase(), password: password.trim());
      _apply(r); return r;
    } catch (_) { _setError('Unexpected error. Please try again.'); return AuthResult.fail(_error!); }
    finally { _end(); }
  }
  ...
  Future<AuthResult> register({required String name, required String email, required String password}) async {
    _begin();
    try {
      final r = await _svc.register(name: name, email: email, password: password);
      _apply(r); return r;
    } catch (_) { _setError('Registration failed. Please try again.'); return AuthResult.fail(_error!); }
    finally { _end(); }
  }
  ...
  void _apply(AuthResult r) {
    if (r.success && r.user != null) {
      _user = r.user; _status = AuthStatus.authenticated; _error = null;
    } else {
      _error = r.error ?? 'Authentication failed.'; _status = AuthStatus.unauthenticated;
    }
  }

  void _setError(String msg) { _error = msg; _status = AuthStatus.unauthenticated; }
}
